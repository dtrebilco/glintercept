...
def create_include_string(extension,extensions):
  ...
  # Get all extensions that are contained in this extension list
  for additional in extension.additionals:
  ...
  if len(tokens) > 0:
    ...
    tokenLength  = 0
    for token in tokens:
      if tokenLength < len(token.name):
        tokenLength = len(token.name)
    
    for token in tokens:
      if token.value[0:2] != "0x":
        header_str.write("//");
      header_str.write("  %s  = %s,\n" % (token.name.ljust(tokenLength), token.value))  
      
    header_str.write("\n};\n\n")    
    
  for function in functions:
    header_str.write("%s %s(%s);\n" % (function.return_type, function.name, create_params_str(function)))
...
def create_extension_header(prefix, extensions, base_dir):
  ...
  for extension in extensions:
    extension.ignored = False

  #Loop and process all contained extensions and mark as already processed
  for extension in extensions:
    for additional in extension.additionals:
      for add in additional:
        for ext in extensions:
  ...
  for extension in extensions:
    if not extension.ignored:
      
      includeFileStr = create_include_string(extension,extensions)
      if len(includeFileStr) > 0:
        ext = extension.name
        post = ext[ext.find("_") + 1 : ]
        postPrefix = post[0 : post.find("_")]
        postPost = post[post.find("_")+1 : ]
