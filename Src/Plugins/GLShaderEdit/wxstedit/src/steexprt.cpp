...
/*
Updated to SciTE 1.68, 2/08/06 - see scite/src/Exporters.cxx
...
*/
    ...
    #pragma implementation "steexprt.h"
#endif
                                      ...
                                      bool overwrite_prompt, bool msg_on_error)
{
    ...
    {
        int overw = wxMessageBox(wxT("Overwrite file : '")+fileName+wxT("'?\n"),
                           wxT("Export error"),
    ...
    if (!ret && msg_on_error)
    {
        wxMessageBox(wxT("Unable to export to file : '")+fileName+wxT("'.\n"),
                     wxT("Export error"),
...
void wxSTEditorExportDialog::OnButton(wxCommandEvent& event)
{
    switch (event.GetId())
    {
        case ID_STEDLG_EXPORT_FILENAME_BITMAPBUTTON :
        {
            ...
            fileName = wxFileSelector( wxT("Export to file"), path, fileName,
                                       extension, wildcards,
                                       wxSAVE, this );
