...
///////////////////////////////////////////////////////////////////////////////
    ...
    #pragma implementation "stefindr.h"
#endif
                                        ...
                                        long style, const wxString& name)
{
    ...
    wxSTEditorFindReplaceSizer(this, true, true);
    ...
    if (HasFlag(STE_FR_NOUPDOWN))
    {
        ...
        FindSizerWindow(m_backwardsCheckBox, GetSizer())->Show(m_backwardsCheckBox, false);
    }

    if (HasFlag(STE_FR_NOMATCHCASE))
    {
        m_matchcaseCheckBox->SetValue(true);
        FindSizerWindow(m_matchcaseCheckBox, GetSizer())->Show(m_matchcaseCheckBox, false);
    }

    if (HasFlag(STE_FR_NOWHOLEWORD))
    {
        m_wholewordCheckBox->SetValue(false);
        FindSizerWindow(m_wholewordCheckBox, GetSizer())->Show(m_wholewordCheckBox, false);
    }

    if (HasFlag(STE_FR_NOWORDSTART))
    {
        m_wordstartCheckBox->SetValue(false);
        FindSizerWindow(m_wordstartCheckBox, GetSizer())->Show(m_wordstartCheckBox, false);
    }

    if (HasFlag(STE_FR_NOWRAPAROUND))
    {
        m_wraparoundCheckBox->SetValue(false);
        FindSizerWindow(m_wraparoundCheckBox, GetSizer())->Show(m_wraparoundCheckBox, false);
    }

    if (HasFlag(STE_FR_NOREGEXP))
    {
        m_regexpFindCheckBox->SetValue(false);
        FindSizerWindow(m_regexpFindCheckBox, GetSizer())->Show(m_regexpFindCheckBox, false);
    }
    ...
    {
        wxSizer *sizer = FindSizerWindow(m_replaceCombo, GetSizer());
        if (sizer)
        {
        ...
        wxSizer *replaceSizer = FindSizerWindow(m_replaceButton, GetSizer());
        sizer = FindSizerSizer(replaceSizer, GetSizer());
        if (sizer)
...
void wxSTEditorFindReplacePanel::OnButton(wxCommandEvent& event)
{
    switch (event.GetId())
    {
        ...
        case ID_STEDLG_FIND_BUTTON        : SendEvent(wxEVT_COMMAND_FIND_NEXT); 
#ifdef GLI_CHANGES
                                            if (!HasFlag(wxFR_REPLACEDIALOG))
                                            {
                                              SendEvent(wxEVT_COMMAND_FIND_CLOSE); event.Skip(); 
                                            }
#endif //GLI_CHANGES
                                            break;
...
void wxSTEditorFindReplacePanel::UpdateFindFlags()
{
    ...
    if ( m_matchcaseCheckBox->GetValue() )
        m_flags |= wxFR_MATCHCASE;
    if ( m_wholewordCheckBox->GetValue() )
        m_flags |= wxFR_WHOLEWORD;
    if ( !m_backwardsCheckBox->GetValue() )
        m_flags |= wxFR_DOWN;

    if ( m_wordstartCheckBox->GetValue() )
        m_flags |= STE_FR_WORDSTART;
    if ( m_regexpFindCheckBox->GetValue() )
        m_flags |= STE_FR_REGEXP;
    if ( m_wraparoundCheckBox->GetValue() )
        m_flags |= STE_FR_WRAPAROUND;

    if ( m_scopewholeRadioButton->GetValue() )
        m_flags |= STE_FR_WHOLEDOC;
    else if ( m_scopecursorRadioButton->GetValue() )
        m_flags |= STE_FR_FROMCURSOR;
    else if ( m_scopealldocsRadioButton->GetValue() )
        m_flags |= STE_FR_ALLDOCS;
}
...
void wxSTEditorFindReplacePanel::OnCheckBox(wxCommandEvent &WXUNUSED(event))
{
    UpdateFindFlags();
    UpdateButtons();
}
...
#ifdef GLI_CHANGES
                           wxDEFAULT_DIALOG_STYLE | style, name) )
#else //GLI_CHANGES
                           wxDEFAULT_DIALOG_STYLE | wxSTAY_ON_TOP | style, name) )
#endif //GLI_CHANGES
...
void wxSTEditorFindReplaceDialog::OnButton(wxCommandEvent& event)
{
    ...
    {
#ifdef GLI_CHANGES
        //Close the dialog on the first search
