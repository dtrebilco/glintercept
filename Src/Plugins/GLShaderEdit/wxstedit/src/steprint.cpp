...
///////////////////////////////////////////////////////////////////////////////
    ...
    #pragma implementation "steprint.h"
#endif
                    ...
                    m_edge_mode(-1)
{
    ...
    //   probably something to two modal dialogs.
    if (sm_warn_on_font_scale && m_editor->GetEditorStyles().Ok())
    {
        ...
        {
            int ret = wxMessageBox(wxT("The font cannot be properly scaled for the printout\n")
                                   wxT("and the output may be corrupted.\n")
                                   wxT("Press cancel to not see this warning again."),
                                   wxT("Unscalable font"),
...
bool wxSTEditorPrintout::OnBeginDocument(int startPage, int endPage)
{
    ...
    // the markers are not drawn anyway so set margin widths to 0
                                     ...
                                     int *pageFrom, int *pageTo)
{
    ...
    *pageTo   = 0;
    ...
    int pages  = 1;
    m_pages.Clear();
    wxProgressDialog progDialog(wxT("Formatting printout"), wxT("Page 1 of ?"));
    ...
    while (pos < length)
    {
        ...
        *maxPage += 1;
        ...
        progDialog.Update(int(pos*100.0/length),
                          wxString::Format(wxT("Page %d of %d"), *maxPage, pages));
    }
...
wxSTEditorPrintOptionsDialog::wxSTEditorPrintOptionsDialog(wxWindow *parent)
                             :wxDialog(parent, -1, _("Printout options"),
