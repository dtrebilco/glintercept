...
*/
    ...
    #pragma implementation "stedit.h"
#endif
...
void wxSTEditor::OnSTCUpdateUI(wxStyledTextEvent &event)
{
    ...
    if (GetEditorPrefs().Ok() && GetEditorPrefs().GetPrefBool(STE_PREF_HIGHLIGHT_BRACES))
        DoBraceMatch();
...
#ifdef GLI_CHANGES
    else if(margin == STE_MARGIN_MARKER)
    {
      //Toggle the breakpoint on a single line click
                                ...
                                int* trans_top_line, int* trans_bottom_line)
{
    ...
    {
        if (top_line < 0)
            top_line = LineFromPosition(GetSelectionStart());
        if (bottom_line < 0)
            bottom_line = LineFromPosition(GetSelectionEnd());
    }
...
size_t wxSTEditor::GetWordCount(int start_pos, int end_pos)
{
    wxString text;
    if (TranslatePos(start_pos, end_pos, &start_pos, &end_pos))
...
bool wxSTEditor::ShowConvertEOLModeDialog()
{
    ...
    wxSingleChoiceDialog dialog(this,
                      wxString(wxT("Current EOL : "))+EOLModeStrings[eol_mode],
                      wxT("Convert End of Line chars"), 3, EOLModeStrings);
...
bool wxSTEditor::ShowSetZoomDialog()
{
    wxNumberEntryDialog numDlg(this,
                               wxT("Scale font sizes : -10...20 (not all fonts supported)"),
                               wxEmptyString,
                               wxT("Change text font size"),
...
bool wxSTEditor::DoFindMatchingBracePosition(int &braceAtCaret, int &braceOpposite, bool sloppy)
{
    ...
    if ((lengthDoc > 0) && (caretPos > 0))
    {
        ...
        if (PositionBefore(caretPos) == (caretPos - 1))
        {
            ...
            styleBefore = static_cast<char>(GetStyleAt(caretPos-1)&31);
        }
    }
    ...
    if ((lengthDoc > 0) && sloppy && (braceAtCaret < 0) && (caretPos < lengthDoc))
    {
        ...
        if (PositionAfter(caretPos) == (caretPos + 1))
        {
            ...
            char styleAfter = static_cast<char>(GetStyleAt(caretPos-1)&31);
...
void wxSTEditor::DoBraceMatch() {
    ...
    {
        char chBrace = static_cast<char>(GetCharAt(braceAtCaret));
...
//~             return false;
        ...
        //ft.chrgText.cpMax = 0;
        int ft_chrg_cpMin = 0;
        int flags = wxSTC_FIND_WORDSTART | (autoCompleteIgnoreCase ? 0 : wxSTC_FIND_MATCHCASE);
                                 ...
                                 int flags)
{
    ...
    if (ok)
    {
        ...
        for (int i = 0; ok && !stream.Eof(); i++)
        {
            ...
            if (i == 0)
            {
                ...
                {
                    int ret = wxMessageBox(wxT("Unicode text file. Convert to Ansi text?"),
                                           wxT("Load Unicode?"),
    ...
    {
        wxMessageBox(wxT("This file is too large for this editor, sorry."),
            wxT("Error loading file"), wxOK|wxICON_EXCLAMATION, parent);
    }
                           ...
                           const wxString &extensions_ )
{
    ...
    if (fileName.IsEmpty())
    {
        ...
        fileName = wxFileSelector(wxT("Open file"), path, fileName,
    ...
    {
        wxMessageBox(wxT("This file is too large for this editor, sorry."),
                     wxT("Error loading file"), wxOK|wxICON_EXCLAMATION, this);
...
bool wxSTEditor::SaveFile( bool use_dialog, const wxString &extensions_ )
{
    ...
    if (fileName.IsEmpty() || use_dialog)
    {
        ...
        fileName = wxFileSelector( wxT("Save file"), path, fileName,
                                   wxEmptyString, extensions,
    ...
    if (!file.IsOpened())
    {
        wxMessageBox(wxT("Error opening file :'") + fileName + wxT("'"),
                     wxT("Save file error"), wxOK|wxICON_ERROR , this);
        return false;
    }
...
bool wxSTEditor::NewFile( const wxString &title_ )
{
    ...
    while (title.IsEmpty())
    {
        title = wxGetTextFromUser(wxT("New file name"), wxT("New file"),
        ...
        if (wxIsWild(title))
        {
            int ret = wxMessageBox(wxT("The filename you specified contains wildcard character.\n")
                                   wxT("Please specify another or cancel to abort."),
                                   wxT("Invalid filename"),
...
int wxSTEditor::QuerySaveIfModified(bool save_file, int style)
{
    ...
    int ret = wxMessageBox(wxT("File : '")+GetFileName()+wxT("' has unsaved changes.\n")
                           wxT("Would you like to save your file before closing?"),
                           wxT("Unsaved changes"),
...
bool wxSTEditor::IsAlteredOnDisk(bool show_reload_dialog)
{
    ...
    if (!fileDT.IsValid())
    {
        ...
        {
            wxMessageBox( _("The file '")+GetFileName()+_T("' doesn't exist on disk anymore."),
    ...
    {
        int ret = wxMessageBox( _("The file '")+GetFileName()+_T("' has been modified externally.\n")
                                _T("Would you like to reload the file?"),
...
void wxSTEditor::UpdateItems(wxMenu *menu, wxMenuBar *menuBar, wxToolBar *toolBar)
{
    ...
    STE_MM::DoSetTextItem(menu, menuBar, ID_STE_PREF_EDGE_COLUMN,
                          wxString::Format(_T("Long l&ine guide column (%d)..."), GetEdgeColumn()));
    ...
    STE_MM::DoSetTextItem(menu, menuBar, ID_STE_PREF_ZOOM,
                          wxString::Format(_T("&Scale font size (%d)..."), GetZoom()));

    // Pref menu items
    STE_MM::DoSetTextItem(menu, menuBar, ID_STE_PREF_TAB_WIDTH,    wxString::Format(_T("Set tab &width (%d)..."), GetTabWidth()));
    STE_MM::DoSetTextItem(menu, menuBar, ID_STE_PREF_INDENT_WIDTH, wxString::Format(_T("Set indent wi&dth (%d)..."), GetIndent()));
    STE_MM::DoSetTextItem(menu, menuBar, ID_STE_PREF_EOL_MODE,     _T("&EOL Mode (")+EOLModeStrings[GetEOLMode()].BeforeFirst(wxT(' '))+wxT(")..."));
...
bool wxSTEditor::HandleMenuEvent(wxCommandEvent& event)
{
    ...
    switch (win_id)
    {
        ...
        case ID_STE_FIND_NEXT :
        {
            ...
            if (!GetParent()->GetEventHandler()->ProcessEvent(event))
                FindString(GetFindString(), GetCurrentPos(), -1, GetFindFlags());
        ...
        {
            int num = wxGetNumberFromUser(wxT("Level to collapse all folds to"),
                                          wxEmptyString, wxT("Collapse folds to level"),
        ...
        {
            int num = wxGetNumberFromUser(wxT("Level to expand all folds to"),
                                          wxEmptyString, wxT("Expand folds to level"),
                                          (GetFoldLevel(GetCurrentLine())&wxSTC_FOLDLEVELNUMBERMASK)-wxSTC_FOLDLEVELBASE,
                                          0, wxSTC_FOLDLEVELNUMBERMASK-wxSTC_FOLDLEVELBASE, this);
            if (num >= 0)
        ...
        //Toggle the beakpoint on the current line
        case ID_STE_BREAKPOINT_TOGGLE  : 
        {
        ...
        case ID_STE_PREFERENCES :
        {
            if (GetEditorPrefs().Ok() || GetEditorStyles().Ok() || GetEditorLangs().Ok())
            {
                ...
                wxSTEditorPrefDialog prefDialog(editorData, this, wxID_ANY, _T("Editor Preferences"),
        ...
        {
            int val = wxGetNumberFromUser(wxT("Column to show long line marker"),
                                          wxEmptyString, wxT("Set long line marker"),
                                          GetEdgeColumn(),
                                          0, 255, this);
            if (val >= 0)
            {
        ...
        {
            int val = wxGetNumberFromUser(wxT("Characters to expand tabs"),
                                          wxEmptyString, wxT("Set tab width"),
                                          GetTabWidth(),
                                          0, 255, this);
            if (val >= 0)
            {
        ...
        {
            int val = wxGetNumberFromUser(wxT("Characters to indent"), wxT(""), wxT("Set indentation width"),
                                          GetIndent(),
                                          0, 255, this);
            if (val >= 0)
            {
        ...
        {
            int eol_mode = GetEOLMode();
            int val = wxGetSingleChoiceIndex(wxString(wxT("Current EOL : "))+EOLModeStrings[eol_mode],
                                             wxT("Select EOL mode"), 3, EOLModeStrings, this);
    ...
    if (show)
    {
        ...
        if (find)
        {
            frDialog = new wxSTEditorFindReplaceDialog(parent,
                               GetFindReplaceData(), _T("Find dialog"),
                               style);
        }
        else // replace
        {
            ...
            frDialog = new wxSTEditorFindReplaceDialog(parent,
                               GetFindReplaceData(), _T("Replace dialog"),
                               style);
        }
...
void wxSTEditor::HandleFindDialogEvent(wxFindDialogEvent& event)
{
    ...
    {
        pos = FindString(findString, pos, -1, flags, true);
                           ...
                           int *found_start_pos, int *found_end_pos)
{
    ...
    else if (STE_HASBIT(flags, STE_FR_WRAPAROUND))
    {
        ...
        if (STE_HASBIT(flags, wxFR_DOWN))
        {
            pos = FindString(findString, 0, -1, flags, hilight,
                             found_start_pos, found_end_pos);
        }
        else
        {
            pos = FindString(findString, textLength, -1, flags, hilight,
                             found_start_pos, found_end_pos);
        }
    }
...
bool wxSTEditor::SelectionIsFindString(const wxString &findString, int flags)
{
    ...
    int find_pos = FindString(findString, sel_start, sel_end, flags, false,
                              &found_start_pos, &found_end_pos);
                                  ...
                                  int flags)
{
    ...
    int found_start_pos = 0;
    int found_end_pos   = 0;
    int pos = FindString(findString, 0, -1, flags, true,
                         &found_start_pos, &found_end_pos);
    ...
    while (pos != -1)
    {
        ...
        pos = FindString(findString, pos + replace_len, -1, flags, true,
                         &found_start_pos, &found_end_pos);
    }
                                  ...
                                  wxArrayInt* endPositions)
{
    ...
    int pos = FindString(str, 0, -1, flags, true, &found_start_pos, &found_end_pos);
    ...
    while (pos != -1)
    {
        ...
        pos = FindString(str, -1, -1, flags, true, &found_start_pos, &found_end_pos);
    }
...
{
    wxString msg = wxString::Format(wxT("Line number : 1...%d"), GetLineCount());
    long line = wxGetNumberFromUser(msg, wxEmptyString, wxT("Goto line"),
...
bool wxSTEditor::ShowPrintDialog()
{
    ...
    wxHtmlEasyPrinting htmlPrint(wxT("Print document"));
    ...
    if (!printer.Print(this, &printout, true))
    {
        if (wxPrinter::GetLastError() == wxPRINTER_ERROR)
        {
            wxMessageBox( _("There was a problem with printing.\n"
                            "Perhaps your current printer is not correctly setup?"),
...
{
#if STE_USE_HTML_PRINT
    wxHtmlEasyPrinting htmlPrint(wxT("Preview document"));
    *htmlPrint.GetPrintData()     = *wxSTEditorPrintout::GetPrintData(true);
    *htmlPrint.GetPageSetupData() = *wxSTEditorPrintout::GetPageSetupData(true);
    wxSTEditorExporter steExport(this);
    ...
    if (!preview->Ok())
    {
        ...
        wxMessageBox( _("There was a problem with previewing.\n"
                        "Perhaps your current printer is not correctly setup?"),
                      _("Preview error"), wxOK|wxICON_ERROR, this);
        return false;
    }
...
bool wxSTEditor::SetLanguage(int lang)
{
    ...
    GetSTERefData()->m_steLangId = lang;
...
{
    return GetSTERefData()->m_steLangId;
}
...
///////////////////////////////////////////////////////////////////////////////
//
void wxSTEditor::ToggleBreakpoint(unsigned int breakLine)
{
  ...
  if(breakLine >= GetLineCount())
  {
    return;
  }

  //Get the raw shader name (strip any path)
  wxFileName fullFileName(GetFileName());

  //Get the current markers at that line
  int state = MarkerGet(breakLine);

  //Toggle the breakpoint state
  if(state & (1 << STE_MARKER_BREAKPOINT))
  {
    MarkerDelete(breakLine, STE_MARKER_BREAKPOINT);

    //If after deleting there are no more markers, flag to GLIntercept that the shader is not debugged
    if(MarkerNext(0, 1<<STE_MARKER_BREAKPOINT) == -1)
    {
      GLIShaderData::GetInstance()->SetDebugShader(false, fullFileName.GetFullName());
    }
  }
  else
  {
    ...
    //Add the breakpoint
    MarkerAdd(breakLine, STE_MARKER_BREAKPOINT);

    //Flag to GLIntercept that debugging on this shader is now enabled
    if(firstBreakpoint)
    {
      GLIShaderData::GetInstance()->SetDebugShader(true, fullFileName.GetFullName());
    }
  }
}
