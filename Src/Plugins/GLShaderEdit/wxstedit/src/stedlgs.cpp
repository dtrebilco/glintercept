...
// Maintainer:
// Created:     2003-04-28
// RCS-ID:      $Id: stedlgs.cpp,v 1.34 2006/04/07 03:42:32 jrl1 Exp $
// Copyright:   (c) John Labenski, Otto Wyss
// Licence:     wxWidgets licence
///////////////////////////////////////////////////////////////////////////////

#if defined(__GNUG__) && !defined(NO_GCC_PRAGMA)
    #pragma implementation "stedlgs.h"
#endif
...
void wxSTEditorPrefDialogPagePrefs::Apply()
{
    ...
    for (n = 0; n < count; n++)
    {
         // may be -1 for not handled or null
        if ((m_prefsToIds[n] < 0) || (FindWindow(m_prefsToIds[n]) == NULL))
            continue;

        // apply only the ones we have
...
BEGIN_EVENT_TABLE(wxSTEditorPrefDialogPageStyles, wxSTEditorPrefDialogPageBase)
    ...
    EVT_STC_MARGINCLICK (wxID_ANY, wxSTEditorPrefDialogPageStyles::OnMarginClick)
    EVT_STE_MARGINDCLICK(wxID_ANY, wxSTEditorPrefDialogPageStyles::OnMarginClick)
    EVT_STC_DOUBLECLICK (wxID_ANY, wxSTEditorPrefDialogPageStyles::OnMarginClick)
END_EVENT_TABLE()
...
void wxSTEditorPrefDialogPageStyles::Init()
{
    ...
    m_helpString =
        ...
        wxT("The list tries to encompass most of the styles for all the language lexers (code parsers). ")
        wxT("Therefore, the names given to the styles are meant only as hints as to what the style will be used for. ")
        wxT("You can examine the lexer styles and change the mapping between the styles in the \"Languages\" tab.\n\n")

        wxT("Select the style you want to change by clicking on the margin. ")
        wxT("You can change the individual attributes for a style by checking the checkboxes to the left or ")
        wxT("uncheck them to use the attribute value from the \"Default Style\". ")
        wxT("Attributes that do not apply for specific styles are disabled.\n\n")

        wxT("The \"Editor Colors\" page allows you to change the overall appearance of the editor. ")
        wxT("The \"Lexer Styles\" page shows the full list of styles to allow you to adjust them all at once to easily create a theme that'll look good for any language. ")
        wxT("The styles that are used for the currently set lexer are shown with a white cross in the margin.");
                               ...
                               :wxSTEditorPrefDialogPageBase(editorPrefData, prefData, parent, winid)
{
    ...
    m_colourEditor  = new wxSTEditor(m_styleNotebook, wxID_ANY);
    m_styleEditor   = new wxSTEditor(m_styleNotebook, wxID_ANY);
    m_helpEditor    = new wxSTEditor(m_styleNotebook, wxID_ANY);

    m_styleNotebook->AddPage(m_colourEditor, wxT("Editor Colors"), true);
    m_styleNotebook->AddPage(m_styleEditor,  wxT("Lexer Styles"),  false);
    m_styleNotebook->AddPage(m_helpEditor,   wxT("Help"),          false);

    m_helpEditor->SetWrapMode(wxSTC_WRAP_WORD);
    m_helpEditor->SetText(m_helpString);
    ...
    if (GetPrefData().GetLangs().Ok())
    {
        ...
        m_langChoice->Append(wxT("Show all styles"));
...
void wxSTEditorPrefDialogPageStyles::OnEvent(wxCommandEvent &event)
{
    ...
    switch (event.GetId())
    {
        ...
        case ID_STEDLG_FONTFORE_BUTTON :
        {
            ...
            dialog.SetTitle(_T("Choose the font's foreground color"));
            if (dialog.ShowModal() == wxID_OK)
            {
        ...
        case ID_STEDLG_FONTBACK_BUTTON :
        {
            ...
            wxColourDialog dialog(this, m_colourData);
            dialog.SetTitle(_T("Choose the font's background color"));
            if (dialog.ShowModal() == wxID_OK)
            {
                *m_colourData = dialog.GetColourData();
                              ...
                              :wxSTEditorPrefDialogPageBase(editorPrefData, prefData, parent, winid)
{
    ...
    m_notebook->AddPage(stylePanel, wxT("Styles"), true);
    ...
    for (n = 0; n < (int)styleArray.GetCount(); n++)
    {
        wxString styleName = steStyles.GetStyleName(styleArray[n]);
        m_styleChoice->Append(styleName, (void*)styleArray[n]);
        if (m_max_stylename_length < (int)styleName.Length())
            m_max_stylename_length = styleName.Length() + 1;
    }
    ...
    m_notebook->AddPage(keywordPanel, wxT("Keywords"), false);
...
void wxSTEditorPrefDialogPageLangs::SetControlValues()
{
    ...
    for (n = 0; n < steLangs.GetKeyWordsCount(m_current_lang); n++)
...
void wxSTEditorPrefDialogPageLangs::OnMarginClick(wxStyledTextEvent &event)
{
    ...
    int line = m_styleEditor->LineFromPosition(event.GetPosition());
                                   ...
                                   long style, const wxString& name )
{
    ...
    if (GetPrefData().GetPrefs().Ok())
    {
        ...
        if (GetPrefData().HasOption(STE_PREF_PAGE_SHOW_VIEW))
        {
            ...
            m_noteBook->AddPage(page, wxT("View"), false);
        }
        if (GetPrefData().HasOption(STE_PREF_PAGE_SHOW_TABSEOL))
        {
            ...
            page->SetControlValues();
            m_noteBook->AddPage(page, wxT("Tabs and EOL"), false);
        }
        if (GetPrefData().HasOption(STE_PREF_PAGE_SHOW_FOLDWRAP))
        {
            ...
            page->SetControlValues();
            m_noteBook->AddPage(page, wxT("Folding and Wrapping"), false);
        }
        if (GetPrefData().HasOption(STE_PREF_PAGE_SHOW_PRINT))
        {
            ...
            page->SetControlValues();
            m_noteBook->AddPage(page, wxT("Printing"), false);
        }
        if (GetPrefData().HasOption(STE_PREF_PAGE_SHOW_LOADSAVE))
        {
            ...
            page->SetControlValues();
            m_noteBook->AddPage(page, wxT("Loading and Saving"), false);
        }
        if (GetPrefData().HasOption(STE_PREF_PAGE_SHOW_HIGHLIGHT))
        {
            ...
            page->SetControlValues();
            m_noteBook->AddPage(page, wxT("Highlighting"), false);
        }
    }
...
#if wxUSE_TOOLTIPS
    resetButton->SetToolTip(wxT("Reset this page's values to their default"));
...
void wxSTEditorPrefDialog::OnApply(wxCommandEvent &event)
{
    ...
    if (GetEditorPrefData().GetEditor())
        GetEditorPrefData().GetEditor()->GetSTERefData()->m_steLangId = GetEditorPrefData().GetLanguageId();
...
void wxSTEditorInsertTextDialog::UpdateControls()
{
    ...
    if (m_type == STE_INSERT_TEXT_ATCOLUMN)
        m_prependText->SetLabel(wxT("Insert"));
    else
        m_prependText->SetLabel(wxT("Prepend"));
}
...
void wxSTEditorColumnizeDialog::FormatText()
{
    ...
    m_testEditor->Columnize(0, -1, splitBefore, splitAfter, preserve);
    m_testEditor->SetReadOnly(true);
}
