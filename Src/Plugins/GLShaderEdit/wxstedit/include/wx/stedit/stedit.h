...
public:
    ...
    ~wxSTERecursionGuard()
    {
        wxASSERT_MSG(m_flag.m_flag > 0, _T("unbalanced wxSTERecursionGuards!?"));
        m_flag.m_flag--;
    }
...
public:
    ...
    int m_steLangId;                // index into the wxSTEditorLangs used
...
public :
    ...
    //  if start_pos == end_pos == -1 use current selection or if none use cursor line
    ...
    //  if top_line == bottom_line == -1 then use top/bottom line of selection
    //    or if no selection use the cursor line
    ...
    // Get the EOL string "\r", "\n", "\r\n" as appropriate for Mac, Unix, DOS
    //  default (-1) gets EOL string for current document settings
    ...
    bool HasState(long ste_statetype) const { return (GetState() & ste_statetype) != 0; }
    void SetStateSingle(long state, bool set) { if (set) SetState(GetState() | state); else SetState(GetState() & ~state); }
...
protected:
    ...
    void ToggleBreakpoint(unsigned int breakLine);
